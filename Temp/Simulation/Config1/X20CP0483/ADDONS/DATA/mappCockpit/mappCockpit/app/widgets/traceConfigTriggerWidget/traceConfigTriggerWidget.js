...
define(["require", "exports", "../common/treeGridWidgetBase", "./view/traceConfigTriggerTreeGridToolbar", "./view/traceConfigTriggerTreeGridCellEditTemplate", "./view/traceConfigTriggerTreeGridCellEditEvents", "../../view/datapointDialog/datapointDialog", "./model/traceConfigTriggerDataModel", "./triggerDescriptionProvider", "./componentDefaultDefinition", "./model/triggerGroup", "../../models/diagnostics/trace/traceConfig/traceConfigValueConverter"], function (require, exports, treeGridWidgetBase_1, traceConfigTriggerTreeGridToolbar_1, traceConfigTriggerTreeGridCellEditTemplate_1, traceConfigTriggerTreeGridCellEditEvents_1, datapointDialog_1, traceConfigTriggerDataModel_1, triggerDescriptionProvider_1, componentDefaultDefinition_1, triggerGroup_1, traceConfigValueConverter_1) {
    ...
    let TraceConfigTriggerWidget = TraceConfigTriggerWidget_1 = class TraceConfigTriggerWidget extends treeGridWidgetBase_1.TreeGridWidgetBase {
         ...
         * @memberof TraceConfigTriggerWidget
         */
        initializeTraceStartTriggerInfo(startTriggers) {
            ...
            traceConfigTriggerDataModel.initData = startTriggers;
        }
        dispose() {
            super.dispose();
        ...
        handleModelChanged(sender, eventArgs) {
            ...
            if (actualSelectedItem == undefined) {
                ...
                let triggerConditionValue = this.getFirstTriggerConditionValue(treeGridObj.model.dataSource);
        ...
        updateFooterContentToSelectedItem(selectedItem) {
            ...
            if (selectedItem.level == 0) {
                // Rootnode selected
                startTriggerGroup = selectedItem.item;
            }
            else {
                // Parameter selected
                startTriggerGroup = selectedItem.parentItem.item;
            }
        ...
        /**
         * Returns the trigger condition(e.g. 20 for IN_WINDOW; 30 for OUT_WINDOW; ...) for the first start trigger if found else 0
         *
         * @param {} dataSource
         * @returns {number}
         * @memberof TraceConfigTriggerWidget
         */
        getFirstTriggerConditionValue(dataSource) {
            for (let i = 0; i < dataSource.length; i++) {
                let startTrigger = dataSource[i];
                for (let j = 0; j < startTrigger.childs.length; j++) {
                    let parameter = startTrigger.childs[j];
                    if (parameter.id == triggerGroup_1.TriggerPropertyIds.Condition) {
                        return traceConfigValueConverter_1.TraceConfigValueConverter.getTriggerConditionIdFromDisplayName(parameter.displayValue);
                    }
                }
            }
            return 0;
        ...
        updateToolbarButtonStates(startTriggers, selectedItem) {
            ...
            if (startTriggers.length < traceConfigTriggerDataModel_1.TraceConfigTriggerDataModel.maxStartTriggerCount) {
