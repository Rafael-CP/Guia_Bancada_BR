...
define(["require", "exports", "./mappCockpitComponent", "./mappCockpitComponentMetaData", "../diagnostics/mappCockpitCommonInfoProvider", "../../widgets/methodParameterListWidget/parameterFilter", "../diagnostics/trace/traceStartTrigger"], function (require, exports, mappCockpitComponent_1, mappCockpitComponentMetaData_1, mappCockpitCommonInfoProvider_1, parameterFilter_1, traceStartTrigger_1) {
    "use strict";
    ...
    let MappCockpitComponentParameterInfo = MappCockpitComponentParameterInfo_1 = class MappCockpitComponentParameterInfo {
        ...
        static retrieveTraceConfigurationTimingParameters(componentParameters) {
            ...
            let currentTraceConfigurationValue = this.getCurrentTraceConfigurationValue(componentParameters);
            if (currentTraceConfigurationValue != "") {
                this.updateTimingParameters(timingParameters, currentTraceConfigurationValue);
            }
        ...
        /**
         * Returns an array of the defined start triggers(max. 15) from the current trace configuration(from CurrentTrcConfig property)
         *
         * @static
         * @param {MappCockpitComponentParameter[]} componentParameters
         * @return {*}  {Array<TraceStartTrigger>}
         * @memberof MappCockpitComponentParameterInfo
         */
        static retrieveTraceConfigurationStartTriggers(componentParameters) {
            let startTriggers = new Array();
            // Update the values to the real values from CurrentTrcConfig Property (BUGFIX for missing StartTriggers at startup)
            let currentTraceConfigurationValue = this.getCurrentTraceConfigurationValue(componentParameters);
            if (currentTraceConfigurationValue != "") {
                startTriggers = this.getStartTriggers(currentTraceConfigurationValue);
            }
            return startTriggers;
        }
        /**
         * Returns an array of the defined start triggers from the current trace configuration JSON string
         *
         * @private
         * @static
         * @param {string} currentTraceConfigJsonString
         ...
         * @memberof MappCockpitComponentParameterInfo
         */
        static getStartTriggers(currentTraceConfigJsonString) {
            let startTriggers = new Array();
            if (currentTraceConfigJsonString != "") {
                let currentTraceConfig = JSON.parse(currentTraceConfigJsonString);
                // Get all available start triggers
                let triggerId = 0;
                let startTrigger = undefined;
                do {
                    triggerId++;
                    startTrigger = this.getStartTrigger(triggerId, currentTraceConfig.Triggers);
                    if (startTrigger != undefined) {
                        startTriggers.push(startTrigger);
                    }
                } while (startTrigger != undefined);
            }
            return startTriggers;
        }
        /**
         * Gets a startTrigger object for the given index from the current trace config data
         *
         * @private
         * @static
         * @param {number} triggerID
         * @param {*} currentTriggerTraceConfig
         * @return {*}  {(TraceStartTrigger|undefined)}
         * @memberof MappCockpitComponentParameterInfo
         */
        static getStartTrigger(triggerID, currentTriggerTraceConfig) {
            let startTrigger = undefined;
            try {
                let singleTriggerCfg = currentTriggerTraceConfig.filter((element) => { return element.ID == triggerID; })[0];
                if (singleTriggerCfg != undefined) {
                    startTrigger = new traceStartTrigger_1.TraceStartTrigger(singleTriggerCfg.Event.toString(), singleTriggerCfg.DataPoint, singleTriggerCfg.Threshold, singleTriggerCfg.Window);
                }
                /*else {
                    // Set Trigger to default if not available in current trace config
                    startTrigger = new TraceStartTrigger("20", "", "0", "0");
                }*/
            }
            catch (error) {
        ...
        /**
         * retrieves the trace configuration datapoints display values from the parameter set
         *
         * @static
         * @param {MappCockpitComponentParameter[]} componentParameters
         * @returns {string[]}
         * @memberof MappCockpitComponentParameterInfo
         */
        static retrieveTraceConfigurationDatapointValues(componentParameters) {
            let datapointDisplayValues = new Array();
            let currentTraceConfigurationValue = this.getCurrentTraceConfigurationValue(componentParameters);
            if (currentTraceConfigurationValue != "") {
                datapointDisplayValues = this.getDataPointNames(currentTraceConfigurationValue);
            }
            return datapointDisplayValues;
        }
        /**
         * Returns a string with the current trace configuration(Property CurrentTrcConfig from trace component) else ""
         *
         * @private
         * @static
         * @param {MappCockpitComponentParameter[]} componentParameters
         ...
         * @memberof MappCockpitComponentParameterInfo
         */
        static getCurrentTraceConfigurationValue(componentParameters) {
            ...
            if (currentTrcConfigProperties.length == 1) {
                return currentTrcConfigProperties[0].value;
            }
            return "";
        }
        /**
         * Returns the values of the datapoint parameters from a json string (currentTraceConfigJsonString)
         *
         * @private
         * @static
         * @param {string} currentTraceConfigJsonString
         ...
         * @memberof MappCockpitComponentParameterInfo
         */
        static getDataPointNames(currentTraceConfigJsonString) {
            let dataPointDisplayValues = new Array();
            if (currentTraceConfigJsonString != "") {
                let currentTraceConfig = JSON.parse(currentTraceConfigJsonString);
                ...
                try {
                    if (currentTraceConfig.DataPoints != undefined) {
                        let dataPointID = 0;
                        let currentDataPoint = undefined;
                        do {
                            dataPointID++;
                            currentDataPoint = currentTraceConfig.DataPoints.filter((element) => { return element.ID == dataPointID; })[0];
                            if (currentDataPoint != undefined) {
                                dataPointDisplayValues.push(currentDataPoint.Name);
                            }
                        } while (currentDataPoint != undefined);
                    }
                }
                catch (error) {
