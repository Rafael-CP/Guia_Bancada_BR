    ...
    let TraceConfigTriggerTreeGridCellEditEvents = class TraceConfigTriggerTreeGridCellEditEvents {
        ...
        endEdit(args) {
            if (args.columnObject.field == "displayValue") {
                ...
                if (triggerParameter.values != undefined) {
                    for (let i = 0; i < triggerParameter.values.length; i++) {
                        let valueItem = triggerParameter.values[i];
                        if (valueItem.displayValue == args.value) {
