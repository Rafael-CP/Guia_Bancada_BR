...
define(["require", "exports", "./driveLogEntryAdditionalData", "./driveLogEntryHelper"], function (require, exports, driveLogEntryAdditionalData_1, driveLogEntryHelper_1) {
    ...
    let DriveLogEntry = class DriveLogEntry {
        ...
        getExportData() {
            ...
            if (this.isErrorRecord() == true && exportData["errorInfo"] == undefined) {
                exportData["errorInfo"] = {}; // Export an empty error info, if no error info is available, to know that this was an error at import(without additional data from target => param definitions)
            }
            return exportData;
        }
        /**
                ...
                if (this._additionalData.mergedData != undefined) {
                    if (this._additionalData.mergedData.errorInfo != undefined) {
                        if (this._linkedRecordType == undefined || this._linkedRecordType == -1 || this._linkedRecordType == 5) { // 5 => "RD_RSP" 
                            ...
                            if (this._recordType == 4 && (this._linkedRecordType == -1 || this._linkedRecordType == undefined)) { // 4 => "RD_REQ"; 
                                // Read Request but no ReadResponse (_linkedRecordType) available => request with a missing response
