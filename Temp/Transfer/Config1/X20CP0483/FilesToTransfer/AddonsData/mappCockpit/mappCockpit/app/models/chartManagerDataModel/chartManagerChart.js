...
define(["require", "exports", "./scale", "../../common/persistence/settings", "./settingIds"], function (require, exports, scale_1, settings_1, settingIds_1) {
    ...
    let ChartManagerChart = class ChartManagerChart {
        ...
        /**
         * Returns a unique id for the next scale, which is currently not used in this chart
         *
         * @returns {string}
         * @memberof ChartManagerChart
         */
        getNextYAxisId() {
            ...
            for (let tryId = 1; tryId < 20; tryId++) {
                newId = idPrefix + tryId.toString();
                for (let i = 0; i < this.childs.length; i++) {
                    let yAxis = this.childs[i];
                    if (yAxis.id == newId) {
                        newId = ""; // Id already used, try new one
                        break;
                    }
                }
                if (newId == "") {
                    continue; // Id already used, try new one
                }
                else {
                    break; // Id not used => exit and return new id
                }
            }
            return newId;
        }
